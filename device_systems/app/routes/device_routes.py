...
@router.get(
    ...
    description="Lista todos los dispositivos con filtros opcionales por tipo, disponibilidad, marca o busqueda.",
...
def get_one_device(device_id: int, db: Session = Depends(get_db)):
...
@router.post(
    ...
    description="Registra un nuevo dispositivo. El numero de serie debe ser unico.",
    response_description="Dispositivo creado",
)
def create(data: DeviceCreate, db: Session = Depends(get_db)):
...
@router.put(
    ...
    description="Reemplaza toda la informacion de un dispositivo.",
    response_description="Dispositivo actualizado",
)
def update(device_id: int, data: DeviceUpdate, db: Session = Depends(get_db)):
    device = device_service.get_device_by_id(db, device_id)
    if not device:
        raise HTTPException(status_code=404, detail="Dispositivo no encontrado")
...
def patch(device_id: int, data: DevicePatch, db: Session = Depends(get_db)):
    device = device_service.get_device_by_id(db, device_id)
    if not device:
        raise HTTPException(status_code=404, detail="Dispositivo no encontrado")
...
def delete(device_id: int, db: Session = Depends(get_db)):
...
@router.get(
    ...
    description="Retorna el historial completo de prestamos de un dispositivo especifico.",
    response_description="Historial de prestamos del dispositivo",
)
